...
const CourseRatings = ({ courseData }) => {
  ...
  return (
    <div className="flex flex-col items-start gap-6 sm:gap-8 lg:gap-10">
      <div className="text-right text-primary text-xl sm:text-2xl md:text-3xl font-bold">
        تقييمات : {average} ({count})
      </div>

      {roundRate && roundRate.length > 0 ? (
        <div className="flex w-full flex-col items-start gap-4 sm:gap-5 lg:gap-7">
          {roundRate.map((rating) => (
            <CoursRateCard key={rating.id} rating={rating} />
          ))}
        </div>
      ) : (
        <div className="text-center py-10 sm:py-12 md:py-16 w-full bg-gray-50 rounded-xl sm:rounded-2xl">
          <p className="text-text-alt text-sm sm:text-base md:text-xl font-medium">
