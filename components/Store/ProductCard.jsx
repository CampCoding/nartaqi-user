  ...
  const renderCartButton = () => {
    ...
    if (isRound) {
      if (isInCart) {
        return (
          <button
            className={`inline-flex items-center justify-center gap-2 px-4 py-2 relative w-full mt-5 md:mt-0 md:w-auto rounded-[10px] transition-all duration-200 cursor-pointer bg-red-500 hover:bg-red-600 text-white ${isLoading ? "opacity-70 cursor-not-allowed" : ""
              }`}
        ...
        return (
          <button
            className={`inline-flex items-center justify-center gap-2 px-4 py-2 relative w-full mt-5 md:mt-0 md:w-auto rounded-[10px] transition-all duration-200 cursor-pointer bg-primary hover:bg-primary-dark text-neutral-50 ${isLoading ? "opacity-70 cursor-not-allowed" : ""
              }`}
    ...
    if (isInCart && localQuantity > 0) {
      return (
        ...
        >
          {/* Decrease Button */}
          <button
            className={`w-10 h-10 flex items-center justify-center rounded-[10px] transition-all duration-200 ${localQuantity <= 1
          ...
          {/* Increase Button */}
          <button
            className={`w-10 h-10 flex items-center justify-center rounded-[10px] bg-primary hover:bg-primary-dark text-white transition-all duration-200 ${isLoading ? "opacity-50 cursor-not-allowed" : ""
              }`}
  ...
  return (
    ...
    >
      ...
      <div className="relative self-stretch w-full h-[180px] md:h-[222px] rounded-[30px_30px_0px_0px] overflow-hidden">
        ...
        {isRound && data.gender && (
          <span
            className={`absolute top-12 right-3 z-10 ${data.gender === "male"
