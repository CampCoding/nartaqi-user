...
import { useGetTopStudentsByCompetitionType } from "../shared/Hooks/getTopStudentsByCompetitionType";
const Leaderboard = () => {
  const [activeTab, setActiveTab] = useState("daily");

  const { data, loading, error, refetch } = useGetTopStudentsByCompetitionType({
    enabled: true,
  });
  ...
  const top1 = useMemo(() => list.find((x) => x.rank === 1) || null, [list]);
  const top2 = useMemo(() => list.find((x) => x.rank === 2) || null, [list]);
  const top3 = useMemo(() => list.find((x) => x.rank === 3) || null, [list]);
    ...
    <div className="w-full min-h-[400px] sm:min-h-[600px] lg:min-h-[670px] pt-8 sm:pt-12 lg:pt-[48px] bg-[url('/images/Leaderboard-section-background.png')] bg-cover bg-center px-4 sm:px-6 lg:px-0">
      <Container className={"flex flex-col items-center"}>
        ...
        <div className="w-full max-w-[1000px] mt-4 flex items-center justify-center">
          {loading ? (
            <div className="text-sm text-text/70">جاري تحميل المتصدرين...</div>
          ) : error ? (
        ...
        <div className="flex flex-col md:flex-row items-center justify-center mt-6 pb-4 md:mt-8 lg:mt-[32px] gap-4 md:gap-2 lg:gap-0 w-full max-w-[1000px]">
          <div className="w-full md:scale-90 order-2 md:order-1">
            <LeaderCard
              ...
              icon={
                <Avatar
                  student={top2}
                  fallback={<UserIcon bgFill={"#F0F0F0"} iconFill={"#C0C0C0"} />}
                />
              }
          ...
          <div className="w-full md:scale-100 order-1 md:order-2">
            <LeaderCard
              color="#FFCB14"
              icon={<Avatar student={top1} fallback={<UserIcon />} />}
              rankLabel={"المركز الاول"}
              student={top1}
            />
          </div>
          ...
          <div className="w-full md:scale-90 order-3">
            <LeaderCard
              ...
              icon={
                <Avatar
                  student={top3}
                  fallback={<UserIcon bgFill={"#FFE7D0"} iconFill={"#CD7F32"} />}
                />
              }
        ...
        <div className="flex items-center gap-1 sm:gap-2 min-w-max">
          {[...tabData].reverse().map((tab) => (
            <button
              ...
              className={[
                "px-4 sm:px-8 lg:px-16 py-3 sm:py-4 rounded-[15px] sm:rounded-[20px] inline-flex items-center justify-center relative flex-shrink-0 transition-all duration-200",
                activeTab === tab.id ? "bg-primary lg:py-5" : "hover:bg-primary/10 lg:py-4",
              ].join(" ")}
              role="tab"
            ...
            >
              <div
                className={[
                  "[display:-webkit-box] justify-center w-fit text-sm sm:text-lg lg:text-xl text-center whitespace-nowrap overflow-hidden text-ellipsis [-webkit-line-clamp:1] [-webkit-box-orient:vertical] [direction:rtl] items-center relative",
...
export const LeaderCard = ({ icon, color, rankLabel, student }) => {
  const name = student?.student_name || "?";
  const points =
    typeof student?.total_points === "number"
  ...
  return (
    <div
      style={{ boxShadow: `0px 0px 50px 10px ${color}70` }}
    ...
    >
      <div className="absolute w-5 h-5 sm:w-6 sm:h-6 top-4 sm:top-5 lg:top-6 right-[15px] sm:right-[16px] lg:right-[17px]">
        <div className="absolute w-[18px] h-[15px] sm:w-[20px] sm:h-[16px] lg:w-[22px] lg:h-[18px] top-[2px] sm:top-[2.5px] lg:top-[3px] left-px">
          <Rank style={{ stroke: color || "#f4a625" }} />
        </div>
      </div>
...
const Avatar = ({ student, fallback }) => {
  ...
  return (
    <img
      ...
      className="w-[120px] h-[120px] sm:w-[140px] sm:h-[140px] lg:w-[160px] lg:h-[160px] rounded-full object-cover"
      loading="lazy"
    />
  );
};
