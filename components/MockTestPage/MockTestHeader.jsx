...
export const MockExamHeader = ({
  ...
  const Controls = () => (
    <div className="flex items-center justify-between sm:justify-center w-full gap-1.5 sm:gap-6 md:gap-10 flex-wrap sm:flex-nowrap pb-1 landscape:pb-0 md:landscape:pb-1">
      {/* Timer */}
      <div className="inline-flex items-center gap-1 sm:gap-2 shrink-0">
        <div className="w-3.5 h-3.5 sm:w-6 sm:h-6 landscape:w-2.5 landscape:h-2.5 md:landscape:w-6 md:landscape:h-6 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full">
          <TimerIcon />
        </div>
        <span className="text-white text-[10px] sm:text-sm md:text-base landscape:text-[9px] md:landscape:text-base font-medium whitespace-nowrap">
      ...
      {!isInReview && (
        <>
          ...
          <button
            className={`inline-flex items-center gap-1 sm:gap-2 shrink-0 px-1.5 sm:px-4 py-0.5 sm:py-1 rounded transition-opacity ${isMarkedForReview ? "bg-white/20" : "hover:bg-white/10"}`}
          ...
          >
            <div className="w-3 h-3 sm:w-5 sm:h-5 landscape:w-2.5 landscape:h-2.5 md:landscape:w-5 md:landscape:h-5 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full">
              <FlagIcon
                className={
                  isMarkedForReview ? "fill-white stroke-white" : "stroke-white"
                }
              />
            </div>
            <span className="text-white text-[10px] sm:text-sm md:text-base landscape:text-[9px] md:landscape:text-base font-medium whitespace-nowrap">
          ...
          <div className="relative shrink-0" ref={selectRef}>
            <button
              className="inline-flex items-center gap-1 sm:gap-6 px-2 sm:px-6 py-0.5 sm:py-1.5 bg-white text-primary rounded-full hover:opacity-90"
              onClick={() => setIsSelectOpen(!isSelectOpen)}
            >
              <div className="w-2.5 h-2.5 sm:w-4 sm:h-4 landscape:w-2 landscape:h-2 md:landscape:w-4 md:landscape:h-4 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full">
                <ChevronDownIcon
                  className={`text-primary transition-transform ${isSelectOpen ? "rotate-180" : ""}`}
                />
              </div>
              <span className="font-bold text-primary text-[9px] sm:text-sm md:text-base landscape:text-[8px] md:landscape:text-base whitespace-nowrap">
                {currentFontSize.label}
              </span>
            </button>
            {isSelectOpen && (
              <div className="absolute top-full left-0 mt-1 bg-white rounded-lg shadow-lg border border-gray-200 z-50 min-w-[100px] sm:min-w-[160px]">
                {fontSizes.map((option) => (
                  <button
                    key={option.value}
                    className={`w-full text-right px-2 sm:px-4 py-1.5 sm:py-3 text-[10px] sm:text-sm landscape:text-[9px] md:landscape:text-sm font-medium hover:bg-gray-50 ${option.value === fontSize ? "bg-blue-50 text-blue-600" : "text-gray-700"}`}
                    onClick={() => {
  ...
  return (
    <header
      className="sticky top-0 bg-primary z-40 px-2 py-1.5 sm:px-6 sm:py-3 landscape:py-0.5 md:landscape:py-3 flex flex-col gap-1 sm:gap-2 shadow-sm w-full"
      dir="rtl"
    >
      <div className="flex justify-center items-center w-full relative">
        <h1 className="font-bold text-white text-[11px] sm:text-xl md:text-2xl landscape:text-[10px] md:landscape:text-2xl truncate max-w-[70%]">
          {examData.examTitle || "الاختبار 1"}
        </h1>
        <div className="hidden md:block absolute right-0 font-medium text-white text-xs sm:text-base truncate max-w-[20%]">
          {examData.studentName}
        </div>
      </div>
      <h3
        className="!text-white text-[11px] sm:text-xl md:text-2xl landscape:text-[10px] landscape:hidden md:landscape:text-2xl !my-0.5 landscape:my-0.5 landscape:py-0.5 !py-0.5 text-center "
        dangerouslySetInnerHTML={{
          __html: currentSection?.title?.replaceAll(/&nbsp;/gi, " ") || "",
        }}
      />
      <nav className="flex w-full mt-0.5 sm:mt-1">
        <Controls />
      </nav>
      {/* Hidden Mobile Controls retained for logic structure */}
      <div className="hidden">
        <Button onClick={() => setDrawerOpen(true)}>عناصر التحكم</Button>
      </div>
    </header>
  );
};
// ==========================================
// SVGs (Unchanged, auto-resizing via parent div)
// ==========================================
